...
from healthnet.models import Account, Action, Hospital, Location, Statistics, Score, Profile
    ...
    if request.method == 'POST':
        ...
        if account is not None:
            if del_:
                ...
                pro.save()
                logger.log(Action.ACTION_ADMIN, 'Admin modified ' + account.user.username + "'s Patient limits", request.user.account)
                template_data['alert_success'] = "Updated " + account.user.username + "'s Patient limits!"
            else:
                account.role = role
                account.save()
                logger.log(Action.ACTION_ADMIN, 'Admin modified ' + account.user.username + "'s role", request.user.account)
