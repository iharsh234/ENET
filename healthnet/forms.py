...
class MedTestForm(BasicForm):
    name = forms.ChoiceField(choices=MedicalTest.TESTS)
    setup_field(name)
    date = forms.DateField()
    setup_field(date)
    hospital = forms.ModelChoiceField(queryset=Hospital.objects.all())
    ...
    setup_field(distance)
    near = forms.BooleanField(required=False)
    setup_field(near)
    ...
    def assign(self, medtest):
        ...
        medtest.distance = self.cleaned_data['distance']
        medtest.near = self.cleaned_data['near']
...
class MedicalInfoForm(BasicForm):
    ...
    vision = forms.ChoiceField(choices=MedicalInfo.VISION)
    setup_field(vision, "Enter vision here")
    glass_prescription =forms.ChoiceField(choices=MedicalInfo.VISION)
    setup_field(glass_prescription)
    dv = forms.ChoiceField(choices=MedicalInfo.VISION)
    setup_field(dv)
    nv = forms.ChoiceField(choices=MedicalInfo.VISION)
    setup_field(nv)
    patching = forms.ChoiceField(choices=MedicalInfo.PATCHING)
    setup_field(patching)
    compliance = forms.ChoiceField(choices=MedicalInfo.DO)
    setup_field(compliance)
    treatment_plan = forms.CharField(max_length=700)
    setup_field(treatment_plan, "Enter Treatment here")
    ...
    def assign(self, medicalInfo):
        ...
        medicalInfo.vision = self.cleaned_data['vision']
        medicalInfo.glass_prescription = self.cleaned_data['glass_prescription']
        medicalInfo.dv  = self.cleaned_data['dv']
        medicalInfo.nv = self.cleaned_data['nv']
        medicalInfo.patching = self.cleaned_data['patching']
        medicalInfo.compliance = self.cleaned_data['compliance']
        medicalInfo.treatment_plan = self.cleaned_data['treatment_plan']
